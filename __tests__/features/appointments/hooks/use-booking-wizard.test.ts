...
vi.mock('@/features/payments/services/client', () => ({
  ...
  verifyPaymentRequest: (...args: unknown[]) => mockVerifyPaymentRequest(...args),
}));
...
describe('useBookingWizard', () => {
  ...
  it('payAndConfirm verifies payment and moves to confirmed', async () => {
    ...
    mockOpenRazorpayCheckout.mockImplementation(async (options: {
      handler: (r: {
    ...
    expect(result.current.state.confirmation?.bookingReference).toBe('HM-ABC123');
  });

  it('payAndConfirm surfaces order creation errors', async () => {
    mockCreatePaymentOrderRequest.mockRejectedValue(new Error('Slot already booked'));
