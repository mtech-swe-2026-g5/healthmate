...
vi.mock(
  "@/features/doctor/appointments/components/PatientAppointments",
  () => ({
    default: vi.fn(({ doctorId }) => (
      <div data-testid="patient-appointments-mock">Doctor ID: {doctorId}</div>
    )),
  }),
);
...
describe("DoctorDashboard", () => {
  ...
  it("should handle empty doctorId string", () => {
    ...
    expect(screen.getByTestId("patient-appointments-mock")).toBeDefined();
    expect(
      screen.getByTestId("patient-appointments-mock").textContent,
    ).toContain("Doctor ID:");
  });
