...
export default function PatientAppointments({doctorId}: PatientAppointmentsProps) {
    ...
    const toAppointmentEvents = useCallback((appointments: Appointment[] | undefined) =>
    ...
    const onAppointmentClick = useCallback((event: AppCalendarEvent) => {
    ...
    return <>
        <AppCalendar className="h-[600]" isLoading={!query.isFetched} events={toAppointmentEvents(query.data)}
                     onRangeChange={onRangeChange} onEventSelect={onAppointmentClick}/>
        <AppointmentModel patient={patientDetails} isModelOpen={isModelOpen} onClose={onClose}/>
    </>;
}